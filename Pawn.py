...
class Pawn(Piece.Piece):
    ...
    def select(self, select):
        ...
        if self.place_y < 8:
            ...
            #field = self.xposmap[self.place_x] + str(self.place_y+2)
        ...
        if self.place_y == 1:
            ...
            #field = self.xposmap[self.place_x] + str(self.place_y+3)
            fieldColor = self.game.board.getColor4Field(field)
            if fieldColor == "":
                self.possibleFields.append(field)
        if self.place_x < 7:
            field = self.getPlace(self.place_x+1,self.place_y+1)
            #field = self.xposmap[self.place_x+1] + str(self.place_y+2)
            fieldColor = self.game.board.getColor4Field(field)
